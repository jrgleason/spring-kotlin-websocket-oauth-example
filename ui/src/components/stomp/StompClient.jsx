...
const StompClient = ({
    ...
    const handleError = (errorMsg) => {
        ...
        console.log(`Error count: ${errorCount.current}/${MAX_ERRORS}`);
        ...
        if (errorCount.current >= MAX_ERRORS) {
            console.log('Max errors reached, stopping retries');
            setHasFailed(true);
        } else {
            console.log(`Scheduling retry attempt ${errorCount.current + 1} in ${RETRY_DELAY}ms`);
            setIsRetrying(true);
    ...
    const initializeStompClient = () => {
        if (hasFailed) {
            console.log('Not initializing due to failed state');
            return;
        }

        cleanup(); // Clean up any existing client
        ...
        const stompClient = new Client({
            ...
            onDisconnect: () => {
                ...
                // Only handle disconnection as an error if we haven't already handled an error
            ...
            onStompError: frame => {
                ...
                const errorMsg = `Error: ${frame.headers?.message || 'Unknown error'}`;
                handleError(errorMsg);
            },
            // Disable automatic reconnect since we're handling it manually
            reconnectDelay: 0
        });
    ...
    useEffect(() => {
        console.log('Initial client setup');
        initializeStompClient();
    ...
    const subscribeToTopic = (stompClient) => {
        ...
        try {
            stompClient.subscribe(
                ...
                {
                    onError: (error) => {
                        const errorMsg = `Subscription error: ${error.headers?.message || 'Access denied'}`;
                        handleError(errorMsg);
                    }
                }
            );
